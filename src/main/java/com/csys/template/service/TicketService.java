...
public class TicketService {
    ...
    public TicketResponseDTO update(Integer ticketId, TicketRequestDTO ticketRequestDTO) {
        ...
        public List<TicketResponseDTO> searchByNaturalLanguage(String query) {
            ...
            Map<String, String> requestBody = Collections.singletonMap("query", query); // <-- Cette ligne doit être corrigée
        ...
        return tickets.stream()
                ...
                .map(t -> {
                    ...
                    // Pour FullCalendar, si c'est une échéance sans durée, start et end peuvent être les mêmes.
                    // Si vous voulez que l'événement s'étende sur toute la journée, allDay: true et pas de temps.
                    // Si c'est un événement ponctuel d'une heure, vous pouvez ajouter 1 heure à la date de fin.
                    ...
                    // Vous pouvez aussi ajouter d'autres données pour un clic détaillé sur le frontend
                    event.put("extendedProps", Map.of(
                        "ticketStatus", t.getStatue().toString(),
                        "ticketPriority", t.getPriorite().toString(),
                        "assignedTo", (t.getIdUtilisateur() != null ? t.getIdUtilisateur().getPrenom() + " " + t.getIdUtilisateur().getNom() : "Non assigné")
                    ));
    ...
    public List<Map<String, Object>> getPerformanceStats(String groupBy, String period) {
        ...
        // IMPORTANT: Pour la date de fin, vous devrez utiliser un champ qui indique quand le ticket a été "terminé".
        // Si vous n'avez que date_echeance, ce n'est pas idéal pour la performance.
        // Je vais filtrer sur dateCreation pour l'exemple, si vous n'avez pas de date de clôture.
        // Idéalement, vous auriez `ticketRepository.findByStatueAndDateClotureAfter(Status.Termine, startDate);`
        ...
        if ("employee".equalsIgnoreCase(groupBy)) {
            ...
            // C'est un peu plus complexe car le ticket est lié à un module, et le module à une équipe.
            groupedCounts = completedTicketsInPeriod.stream()
            ...
            throw new IllegalArgumentException("Invalid groupBy parameter for performance stats. Must be 'employee' or 'team'.");
        }
    ...
    public List<TicketResponseDTO> getOverdueTickets() {
        ...
        // Convertir en DTOs de réponse légers pour éviter les boucles infinies ou trop de données
